...
passport.deserializeUser(async (userID, done) => {
    try {
        const user = await client.db("cs4241-a3").collection("users").findOne({ _id: new ObjectId(userID) });
...
passport.use(new LocalStrategy({
    ...
    try {
        ...
        const user = await client.db("cs4241-a3").collection("users").findOne({ email: email });
        if (!user) throw new Error("User not found");
